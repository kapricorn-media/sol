...
const State = struct {
    ...
    pub fn init() !Self
    {
        return Self {
            ...
            .quadTexture = try QuadTextureState.init(),
...
const QuadState = struct {
    ...
    pub fn init() !Self
    {
        ...
        const vertQuadId = compileShader(&vert[0], vert.len, GL_VERTEX_SHADER);
        const fragQuadId = compileShader(&frag[0], frag.len, GL_FRAGMENT_SHADER);
        ...
        const colorUniLoc = glGetUniformLocation(programId, &u_color[0], u_color.len);
        if (colorUniLoc == -1) {
            return error.MissingUniformLoc;
        }
        ...
        color: m.Vec4) void
    {
        ...
        glUniform4fv(self.colorUniLoc, color.x, color.y, color.z, color.w);
...
export fn onAnimationFrame(width: c_int, height: c_int, timestampMs: c_int) void
{
    ...
    const pos = m.Vec2.init(50, 50);
    const size = m.Vec2.init(50, 50);
    const color = m.Vec4.init(0.0, 0.0, 0.0, 1.0);
    _state.quad.drawQuad(pos, size, color, screenSizeF);
